...
static inline IMPPayloadType map_payload(hal_payload_t pt) {
    switch (pt) {
        ...
        case HAL_PT_H265: return PT_H265;
        case HAL_PT_H264:
...
int hal_stream_release(int ch, hal_stream_t *s) {
    ...
    free(s->impl); s->impl = NULL; return ret;
}

static inline IMPEncoderStream* as_native(const hal_stream_t *s) { return (IMPEncoderStream*)s->impl; }
