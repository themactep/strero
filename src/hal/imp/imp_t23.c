...
int hal_enc_create(int ch, const hal_enc_attr_t *a) {
    ...
    /* Minimal sane defaults: follow existing T23 code which relies on SDK defaults.
       Do not set per-mode fields unless necessary; IMP provides defaults. */
...
int hal_enc_unregister(int group, int ch) {
    (void)group; /* T23 UnRegister takes only channel */
...
/* ---------------- Optional tuning (mostly unsupported on T23) ---------------- */
...
int hal_enc_set_stream_bufsize(int ch, uint32_t bytes) {
    (void)ch; (void)bytes;
    return 0; /* no-op: not available on T23 public SDKs */
}
